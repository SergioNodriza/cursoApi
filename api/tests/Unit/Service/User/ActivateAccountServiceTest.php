...
class ActivateAccountServiceTest extends UserServiceTestBase
{
    ...
    public function testForNonExistingUser(): void
    {
        ...
        $this->userRepository
            ...
            ->method('findOneInactiveByIdAndTokenOrFail')
            ->with($id, $token)
            ->willThrowException(new UserNotFoundException(\sprintf('User with %s and token %s not found', $id, $token)));

        $this->expectException(UserNotFoundException::class);
        $this->expectExceptionMessage(\sprintf('User with %s and token %s not found', $id, $token));
