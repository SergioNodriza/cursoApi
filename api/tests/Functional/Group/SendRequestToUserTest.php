...
class SendRequestToUserTest extends GroupTestBase
{
    public function testSendRequestToUser(): void
    {
        ...
        self::$peter->request('PUT', \sprintf('%s/%s/send_request', $this->endpoint, $this->getPeterGroupId()), [], [], [], \json_encode($payload));
    ...
    public function testSendAnotherGroupRequestToUser()
    {
        $payload = ['email' => 'roger@api.com'];

        self::$brian->request('PUT', \sprintf('%s/%s/send_request', $this->endpoint, $this->getPeterGroupId()), [], [], [], \json_encode($payload));
